...
export function FindOrderSheet({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const submit = () => {
    const next: typeof errors = {};
    if (!normalizeOrderNumber(number)) next.number = "Enter the order number from your WhatsApp message, like DNF-1041.";
    if (!normalizeGhPhone(phone)) next.phone = PHONE_ERROR;
      ...
      {stage === "code" && orderId ? (
        ...
        >
          ...
          <div className="field">
            ...
            <input id="find-number" autoCapitalize="characters" placeholder="DNF-1041" value={number} onChange={(e) => setNumber(e.target.value)} aria-invalid={Boolean(errors.number)} aria-describedby="find-number-hint" style={{ fontFamily: "var(--mono)" }} />
            {errors.number && (
