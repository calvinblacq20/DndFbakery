...
type Variant = "outline" | "soft" | "dark" | "berry" | "danger" | "ghost-dark";
...
interface CtaProps {
  ...
  type?: "button" | "submit";
  className?: string;
  /** "dark" on light backgrounds (default); "berry" on dark sections, as in the reference. */
  tone?: "dark" | "berry";
}
...
export function Cta({ children, onClick, loading, disabled, type = "button", className = "", tone = "dark" }: CtaProps) {
  ...
  const base = tone === "berry" ? BERRY : INK;
  const flip = tone === "berry" ? INK : BERRY;
  return (
    <motion.button
      type={type}
      ...
      whileTap={{ scale: 0.98 }}
      animate={{ backgroundColor: disabled ? "rgba(43, 31, 29,0.25)" : active ? flip : base, color: (active ? flip : base) === INK ? "#ffffff" : INK }}
