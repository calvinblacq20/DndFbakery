...
/** Birthdays and anniversaries the client wants remembered, plus what the kitchen should know. */
                  ...
                  <motion.div key={c.id} layout exit={{ opacity: 0, x: -24 }} transition={spring.small}>
                    <Reveal as="section" className="card card-pad stack gap-12" delay={i * 0.05}>
                      <div className="between" style={{ alignItems: "flex-start", gap: 12 }}>
                        ...
                        {c.days <= 14 && (
                          <span className="badge is-berry" style={{ paddingRight: 10 }}>
...
function DietarySheet({ open, onClose, initial }: { open: boolean; onClose: () => void; initial: string }) {
  ...
  return (
    <Sheet open={open} onClose={onClose} title="Allergies and dietary needs">
      <div className="stack gap-16">
        <div className="field">
          <label htmlFor="dietary-text">What should the kitchen know?</label>
          ...
          <span className="hint">Our kitchen uses eggs, milk, wheat and nuts. We'll note this on every order.</span>
        </div>
        {error && (
          <p className="t-cap" role="alert" style={{ color: "var(--danger)" }}>
            {error}
        ...
        <Button
          ...
          onClick={() => {
            ...
            notify("Saved", "The kitchen will see this on your orders.");
            onClose();
          }}
        >
          Save
