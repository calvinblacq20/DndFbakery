      ...
      {loading ? (
        ...
        <AnimatePresence mode="wait">
          <motion.div key={tab} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={spring.small}>
            ...
            {tab === "orders" && (
              orders.length === 0 ? (
                <Empty
                  ...
                  body={account ? "Orders you place with the bakery will show here." : "Ordered on another phone? Find it with your order number and WhatsApp number."}
                  cta={{ to: "/order/new", label: "Order a cake" }}
                />
              ) : (
            ...
            {tab === "pickups" && (
              upcoming.length === 0 ? (
                <Empty icon={<CalendarDays size={24} />} title="Nothing coming up" body="Pickups, deliveries and design sessions you book will show here." cta={{ to: "/order/new", label: "Order a cake" }} />
              ) : (
