import { CakeSlice, CalendarClock, Clock, Gift, GraduationCap, Heart, MapPin, MessageCircle, Palette, Pause, Phone, Play, Share2, Sparkles, Truck, Users, Wallet } from "lucide-react";
...
const FEATURE_ICONS = { truck: Truck, users: Users, school: GraduationCap, cake: CakeSlice, wallet: Wallet, calendar: CalendarClock } as const;
...
const STATEMENT =
  "Dough n Frost is Em's home bakery in Adenta. Celebration cakes, pies, doughnuts, rock cakes and pastries, baked to order for special occasions, everyday treats, or just because.";
const HIGHLIGHTS = [
  { icon: Palette, title: "Your design", note: "From Frozen to Adinkra, made by hand" },
  { icon: Truck, title: "Pickup or delivery", note: "Adenta pickup, delivery across Accra" },
  { icon: Wallet, title: "Clear prices", note: "Our price list, up front" },
];
...
function HomeSkeleton() {
  return (
    <main className="screen" aria-busy="true" aria-label="Loading the bakery">
...
function StudioPage() {
  ...
  const cheapest = Math.min(...STYLES.map((s) => fromPriceOf(s)));
  ...
  const share = async () => {
    const shareData = { title: STUDIO.name, text: `${STUDIO.name}: ${STUDIO.tagline} in Adenta`, url: window.location.href };
    try {
      if (navigator.share) await navigator.share(shareData);
      else {
        await navigator.clipboard.writeText(shareData.url);
        notify("Link copied", "Paste it anywhere to share the bakery.");
      }
    } catch {
  ...
  const actionButtons = (
    <>
      <button className="icon-btn" onClick={share} aria-label="Share the bakery">
        <Share2 size={18} strokeWidth={1.8} />
      </button>
      <motion.button className={`icon-btn ${saved ? "is-on" : ""}`} onClick={() => setSaved(!saved)} aria-pressed={saved} aria-label="Save the bakery" whileTap={{ scale: 0.85 }} transition={spring.press}>
  ...
  return (
    <main className="screen studio">
        ...
        <AnimatePresence>
          {showHeader && (
            <motion.div className="overlay-header-inner" initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={spring.micro}>
              ...
              <nav className="section-tabs" aria-label="Bakery sections">
                {SECTIONS.map((s) => (
      ...
      <nav className="section-tabs desk-section-tabs desktop-only" aria-label="Bakery sections">
        {SECTIONS.map((s) => (
          <button key={s.id} className={`section-tab ${active === s.id ? "is-active" : ""}`} onClick={() => goTo(s.id)}>
            {s.label}
      ...
      <section id="lookbook" className="section anchor">
        ...
        <Marquee label="Gallery" className="looks" direction="right" paused={lookbookPaused}>
          {OCCASIONS.filter((o) => o.id !== "other").map((o, i) => (
            <Link key={o.id} to={`/explore?occasion=${o.id}`} className="look-card" aria-label={`${o.label} cakes and treats`} draggable={false}>
      ...
      <section id="about" className="section anchor about">
        <Reveal as="span" look="focus" className="chip-soft">
          <CakeSlice size={13} /> About the bakery
        </Reveal>
        ...
        <button className="link t-body" onClick={() => setAboutOpen(!aboutOpen)} style={{ alignSelf: "flex-start" }}>
          {aboutOpen ? "Show less" : "More about the bakery"}
        </button>
      </section>
      ...
      <aside className="studio-aside desk" aria-label="Order from the bakery">
        <div className="card aside-card stack gap-12">
          ...
          <p className="info-line">
            <Truck size={16} />
            <span>Pickup in Adenta · delivery across Accra</span>
          </p>
          <p className="info-line">
            <CakeSlice size={16} />
            <span>
              {STYLES.length} cakes and treats from {money(cheapest)}
            </span>
          </p>
        ...
        <div className="card card-pad stack gap-8">
          <Photo tone="mist" src="/photos/wedding-reception.webp" alt="A Dough n Frost wedding cake and cupcake tower at a reception" position="center 45%" sizes="320px" height={150} radius="var(--r-img)" />
          <p className="t-title">Weddings and events</p>
          <p className="muted">Planning a wedding, a naming ceremony or an office launch? Book a design and tasting session, and add small chops and snack boxes for your guests.</p>
          <Link className="link t-body" to="/order/new?style=wedding-cake" style={{ alignSelf: "flex-start" }}>
            Plan a wedding cake
          </Link>
        </div>
      </aside>
        ...
        <div className="card list-card">
          ...
          <a className="row" href={STUDIO.instagram} target="_blank" rel="noreferrer">
            ...
            <span className="grow stack">
              <span>@dough_n_frost on Instagram</span>
      ...
      <section className="section">
        ...
        <div className="card list-card">
          ...
          <a className="row" href={whatsappLink("", `I get my cakes from ${STUDIO.name} in Adenta. Order here: ${window.location.origin}`)} target="_blank" rel="noreferrer">
            <span className="row-icon">
      ...
      <div className="floating-bar lt-desk">
        <div className="sticky-bar-meta">
          <strong>{STYLES.length} cakes and treats</strong>
