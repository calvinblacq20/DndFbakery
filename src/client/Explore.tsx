  ...
  return (
    <main className="screen">
      <header className="page-title" style={{ paddingTop: 20 }}>
        ...
        <p className="muted">{CATEGORIES.length} categories · baked to order in Adenta</p>
      </header>
      ...
      <div className="stack gap-12 explore-bar" style={{ position: "sticky", top: 0, zIndex: 10, background: "var(--ground)", marginInline: "calc(var(--gutter) * -1)", padding: "8px var(--gutter) 10px" }}>
        <label className="search-box">
          ...
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search red velvet, cupcakes, wedding…" inputMode="search" />
          {query && (
            ...
            {results.map((style, i) => {
              ...
              return (
                <motion.div key={style.id} layout="position" exit={{ opacity: 0, scale: 0.96 }} transition={spring.small} className="style-tile">
                  ...
                  <button onClick={() => navigate(`/order/new?style=${style.id}`)} className="stack gap-8" style={{ textAlign: "left", width: "100%" }} aria-label={`Order ${style.name}, from ${money(fromPriceOf(style))}`}>
                    ...
                    <span className="stack">
                      ...
                      <span className="tabular" style={{ fontWeight: 500 }}>{style.kind === "unit" ? money(style.fromPrice) : `from ${money(fromPriceOf(style))}`}</span>
                    </span>
                  </button>
