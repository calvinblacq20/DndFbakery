...
import type { ContactDetails, Delivery, DesignPlan, Occasion, OrderStatus, PayChoice, Style } from "../data/types";
...
import { FINISH_ADD, defaultChoice, estimate, fixChoice, fromPriceOf, gridPrice, hasFlavour, layersFor, sizesFor, sizesOf, unitPrice, type ItemChoice } from "../lib/pricing";
import { dateStrip, isOpenDay, neededByFit, readyWindow, slotsFor } from "../lib/schedule";
import { spring } from "../motion";

type Line = ItemChoice;
type PricedLine = Line & { style?: Style; unitPrice: number };

const STEP_TITLES = ["Choose your treats", "Customise", "Date and pickup", "Your details", "Review and pay"] as const;
...
const newLine = (style: Style): Line => ({ styleId: style.id, ...defaultChoice(style) });
...
export function OrderFlow() {
  ...
  const needsQuote = custom || designPlan === "consult";
  ...
  const missing = (() => {
    ...
    if (step === 2) {
      ...
      if (designPlan === "consult" && !consultStart) return "Pick a design session";
    }
  ...
  const updateLine = (styleId: string, patch: Partial<Line>) =>
    setLines((prev) =>
      prev.map((l) => {
  ...
  const placeOrder = (payment?: OnlinePayment): string | null => {
    ...
    const result = actions.placeOrder({
      ...
      shortNotice,
      total,
      items: priced.map(({ styleId, qty, flavour, size, layers, tiers, finish, design, message, unitPrice: u }) => ({ styleId, qty, flavour, size, layers, tiers, finish, design, message: message?.trim() || undefined, unitPrice: u })),
    ...
    <main className="screen flow-screen">
      ...
      <div className="flow-layout">
            ...
            <motion.div key={step} initial={{ opacity: 0, x: 28 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -28 }} transition={spring.small} className="stack" style={{ flex: 1 }}>
              ...
              {step === 2 && (
                <DateAndHandover
                  ...
                  takenSlots={data.appointments.filter((a) => a.status !== "cancelled" && (a.purpose === "consultation" || a.purpose === "tasting")).map((a) => a.start)}
                  delivery={delivery}
                  setDelivery={(d) => {
                    setDelivery(d);
                    setHandoverStart(null);
        ...
        <aside className="flow-aside desk" aria-label="Order summary">
          <div className="card aside-card stack gap-12">
            ...
            {consultStart && designPlan === "consult" && (
              <p className="info-line t-cap muted">
                <Clock size={14} />
                <span>
                  Design session {fmtDayShort(parseLocal(consultStart))}, {fmtTime(parseLocal(consultStart))}
                </span>
              </p>
            )}
            {est.lateFee > 0 && (
            ...
            <div className="kv kv-total">
              <span>{needsQuote ? "Estimated total" : "Total"}</span>
          ...
          <p className="t-cap subtle" style={{ textAlign: "center" }}>
            Full payment secures your date. Custom designs are confirmed with a quote.
          </p>
        </aside>
      </div>
      ...
      <div className="sticky-bar lt-desk">
        <div className="sticky-bar-meta">
          {step === REVIEW_STEP ? (
            <>
              <span className="subtle t-cap">{pay === "now" ? "Pay today" : "Estimated total"}</span>
...
function ChooseItems({ lines, onToggle, onQty }: { lines: Line[]; onToggle: (style: Style) => void; onQty: (id: string, qty: number) => void }) {
        ...
        <section className="section" style={{ marginTop: 12 }}>
          ...
          {custom ? (
            <div className="stack gap-12" role="radiogroup" aria-label="How we get your design">
              ...
              <PlanCard selected={designPlan === "consult"} onSelect={() => setDesignPlan("consult")} icon={<CakeSlice size={20} />} title="Book a design and tasting session" body={`45 minutes at ${STUDIO.area}. Best for wedding cakes.`} />
            </div>
          ) : (
          ...
          <label className="upload">
            <span className="row-icon is-berry">
              <ImagePlus size={18} />
            </span>
...
function LineOptions({ line, style, onUpdate }: { line: PricedLine; style: Style; onUpdate: (patch: Partial<Line>) => void }) {
  const min = style.minQty ?? 1;
  const layerOptions = style.kind === "cake" ? layersFor(style, line.flavour) : [];
  const sizeOptions = style.kind === "cake" && line.layers ? sizesFor(style, line.flavour, line.layers) : [];
    ...
    <section className="card card-pad stack gap-16">
      ...
      {hasFlavour(style) && (
        <OptionGroup label="Flavour">
          <div className="chips" style={{ flexWrap: "wrap", marginInline: 0, paddingInline: 0 }} role="radiogroup" aria-label={`Flavour for ${style.name}`}>
            {FLAVOURS.map((f) => (
              <button key={f.id} role="radio" aria-checked={line.flavour === f.id} className={`chip ${line.flavour === f.id ? "is-active" : ""}`} onClick={() => onUpdate({ flavour: f.id })}>
                {f.label}
              </button>
            ))}
          </div>
        ...
        <>
          <OptionGroup label="Layers">
            <div className="segmented" role="radiogroup" aria-label={`Layers for ${style.name}`}>
              {LAYER_OPTIONS.map((o) => (
                <button key={o.id} role="radio" aria-checked={line.layers === o.id} className={line.layers === o.id ? "is-active" : ""} disabled={!layerOptions.includes(o.id)} onClick={() => onUpdate({ layers: o.id })}>
                  {o.label}
                </button>
              ))}
            </div>
          ...
          <OptionGroup label="Size">
            <div className="option-grid" role="radiogroup" aria-label={`Size for ${style.name}`}>
              {sizesOf(style).map((id) => {
                ...
                const price = line.layers ? gridPrice(line.flavour, line.layers, id) : null;
      ...
      {style.kind !== "unit" && (
        <div className="field">
          ...
          <span id={`msg-${style.id}-hint`} className="hint">
            {(line.message ?? "").length}/{MESSAGE_MAX} · piped in chocolate or on a topper
          </span>
        </div>
      )}

      {style.kind === "unit" && (
        <OptionGroup label={`How many (${style.unit ?? "each"})`}>
      ...
      {p.consult && (
        <motion.section className="section" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={spring.small}>
          ...
          {consultDay && !slotsLoading && (
            <div className="slot-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8 }} role="radiogroup" aria-label="Design session time">
              {consultSlots.map((s) => (
...
function PlanCard({ selected, onSelect, icon, title, body, disabled }: { selected: boolean; onSelect: () => void; icon: ReactNode; title: string; body: string; disabled?: boolean }) {
  return (
    <button role="radio" aria-checked={selected} className={`select-card ${selected ? "is-selected" : ""}`} onClick={onSelect} disabled={disabled} style={{ alignItems: "center" }}>
      <span className="row-icon" style={{ width: 44, height: 44, borderRadius: 999, background: selected ? "var(--berry)" : undefined }}>
        {icon}
      </span>
      <span className="grow stack">
...
const PLAN_TEXT: Record<DesignPlan, string> = {
  ...
  consult: "Design and tasting session",
};
    ...
    <div className="stack gap-8">
      ...
      <section className="section">
        ...
        <p className="t-cap subtle">
          {r.needsQuote ? `Prices are estimates until ${STUDIO.name} confirms your design.` : "Prices come from our price list."}
