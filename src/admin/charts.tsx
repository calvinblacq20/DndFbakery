        ...
        <svg width={width} height={height} aria-hidden="true">
          <defs>
            <linearGradient id={gradientId} x1="0" x2="0" y1="0" y2="1">
              <stop offset="0%" stopColor="#d4c1ff" stopOpacity="0.28" />
              <stop offset="100%" stopColor="#d4c1ff" stopOpacity="0" />
            </linearGradient>
          </defs>
          {ticks.map((t) => (
            <g key={t}>
              <line x1={left} x2={width - right} y1={y(t)} y2={y(t)} style={{ stroke: "rgba(43, 31, 29,0.06)" }} />
          ...
          {hover !== null && !empty && (
            <g>
              <line x1={x(hover)} x2={x(hover)} y1={top} y2={top + innerH} style={{ stroke: "rgba(43, 31, 29,0.12)" }} />
              {series.map((s) => {
...
export function BarChart({ bars, ariaLabel, height = 150, animate = true, format = String }: { bars: Bar[]; ariaLabel: string; height?: number; animate?: boolean; format?: (n: number) => string }) {
  ...
  return (
    <div ref={ref} className="chart" role="img" aria-label={ariaLabel} style={{ height }} onPointerLeave={() => setHover(null)}>
      {width > 0 && (
        <svg width={width} height={height} aria-hidden="true">
          <line x1={0} x2={width} y1={top + innerH} y2={top + innerH} style={{ stroke: "rgba(43, 31, 29,0.06)" }} />
          {bars.map((b, i) => {
            ...
            const color = b.closed || b.value === 0 ? "rgba(43, 31, 29,0.06)" : b.emphasis || hover === i ? "#2b1f1d" : "#8a5a4c";
            return (
              <g key={i} onPointerEnter={() => setHover(i)}>
                ...
                />
                {b.value > 0 && !b.closed && (
                  <text x={cx} y={y0 - 5} textAnchor="middle" style={{ fontSize: 11, fill: "#2b1f1d", fontWeight: 500 }}>
                    {format(b.value)}
                  </text>
                )}
                {i % labelEvery === 0 && (
...
export function Sparkline({ values, width = 72, height = 28, label }: { values: (number | null)[]; width?: number; height?: number; label: string }) {
  ...
  return (
    <svg width={width} height={height} role="img" aria-label={label} style={{ flex: "none", overflow: "visible" }}>
      <path d={d} style={{ fill: "none", stroke: "#8a5a4c", strokeWidth: 1.5, strokeLinejoin: "round", strokeLinecap: "round" }} />
      {end && <circle cx={end[0]} cy={end[1]} r={2.5} style={{ fill: "#2b1f1d" }} />}
    </svg>
  );
}
