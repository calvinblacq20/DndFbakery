...
type Line = Omit<ItemChoice, "styleId"> & { key: number; styleId: string };
...
const PLANS: { id: DesignPlan; label: string }[] = [
  ...
  { id: "consult", label: "Design session first" },
];

const toNumber = (v: string) => Number(v.replace(/[^\d.]/g, ""));
const emptyLine = (key: number): Line => ({ key, styleId: "", qty: 1, flavour: "vanilla", finish: "buttercream", design: "classic" });
...
const styleOptions = (styles: Style[]): DropdownOption<string>[] =>
  CATEGORIES.flatMap((c) =>
    styles
      ...
      .map((s) => ({ value: s.id, label: s.name, hint: s.kind === "unit" ? money(s.fromPrice) : `from ${money(fromPriceOf(s))}`, group: c.label })),
  );
...
export function NewOrder() {
  ...
  const [client, setClient] = useState({ name: "", phone: "", town: "Adenta", source: "whatsapp" as LeadSource });
  ...
  const submit = (e: FormEvent) => {
    ...
    if (closedDay) return setError("The bakery is closed that day. Pick another day.");
    if (fit === "too-soon") return setError("That's too soon to bake. Agree a later day with the client.");
    ...
    const draft: WalkInOrder = {
      ...
      readyBy: neededBy,
      shortNotice,
      items: priced.map((p) => ({ styleId: p.styleId, qty: p.qty, flavour: p.flavour, size: p.size, layers: p.layers, tiers: p.tiers, finish: p.finish, design: p.design, message: p.message?.trim() || undefined, unitPrice: p.unitPrice })),
      total,
            ...
            <div className="adm-card-body stack gap-12">
              {mode === "existing" ? (
                ...
                <div className="adm-grid adm-grid-2" style={{ gap: 16 }}>
                  ...
                  <div className="field">
                    <label htmlFor="nc-source">Found the bakery through</label>
              ...
              {closedDay && (
                <p className="banner is-sand" role="status">
                  <span className="inline" style={{ gap: 8 }}>
                    <TriangleAlert size={16} /> The bakery is closed that day.
                  </span>
                </p>
              )}
              {fit === "late" && !closedDay && (
...
function LineFields({ line, index, styles, canRemove, onStyle, onChange, onRemove }: { line: Line; index: number; styles: DropdownOption<string>[]; canRemove: boolean; onStyle: (id: string) => void; onChange: (change: Partial<Line>) => void; onRemove: () => void }) {
    ...
    <fieldset className="stack gap-12" style={{ border: 0, padding: index ? "16px 0 0" : 0, margin: 0, borderTop: index ? "1px solid var(--ink-06)" : undefined }}>
      ...
      {style && (
        <div className="adm-grid adm-grid-3" style={{ gap: 12 }}>
          {hasFlavour(style) && (
            <div className="field">
              <label htmlFor={`flavour-${line.key}`}>Flavour</label>
              <Dropdown<Flavour> id={`flavour-${line.key}`} variant="field" value={line.flavour} onChange={(flavour) => onChange({ flavour })} options={FLAVOURS.map((f) => ({ value: f.id, label: f.label }))} />
            </div>
          )}
          {style.kind === "cake" && (
            <>
              <div className="field">
                ...
                <Dropdown<string> id={`layers-${line.key}`} variant="field" value={String(line.layers ?? "")} onChange={(v) => onChange({ layers: Number(v) as 1 | 2 | 3 })} options={LAYER_OPTIONS.filter((o) => layerOptions.includes(o.id)).map((o) => ({ value: String(o.id), label: o.label }))} />
              </div>
              <div className="field">
                ...
                <Dropdown<CakeSize>
                  ...
                  options={sizesOf(style)
                    ...
                    .map((id) => ({ value: id, label: SIZES.find((s) => s.id === id)?.label ?? id, hint: line.layers ? money(gridPrice(line.flavour, line.layers, id) ?? 0) : undefined }))}
                />
              </div>
            </>
          )}
