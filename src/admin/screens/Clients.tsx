...
export function Clients() {
  ...
  const fields = (
    <>
      ...
      <fieldset className="filter-group" style={{ border: 0, padding: 0, margin: "18px 0 0" }}>
        ...
        <CheckRow checked={filters.owes} onChange={(on) => apply({ ...filters, owes: on })} label="Owes the bakery" count={owing.length} />
      ...
      <fieldset className="filter-group" style={{ border: 0, padding: 0, margin: "18px 0 0" }}>
        <legend className="filter-group-label">Found the bakery through</legend>
        {SOURCES.map((s) => (
  ...
  const chips = [
    ...(filters.owes ? [{ key: "owes", label: "Owes the bakery", remove: () => apply({ ...filters, owes: false }) }] : []),
      ...
      <div className="adm-list-layout has-rail">
        ...
        <aside className="adm-rail" aria-label="Balances">
          <section className="adm-card" aria-labelledby="owe">
            <CardHead id="owe" title="Owe the bakery" action={<span className="chip-count">{owing.length}</span>} />
            {owing.length ? (
