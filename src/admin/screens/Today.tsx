...
export const INK = "#2b1f1d";
export const PREVIOUS = "rgba(43, 31, 29,0.5)";

export const PURPOSE_LABEL: Record<Appointment["purpose"], string> = { pickup: "Pickup", delivery: "Delivery", tasting: "Tasting", consultation: "Design session" };
...
export function Today() {
  ...
  const urgent =
    shop.newRequests > 0
      ...
      : shop.late > 0
        ? { title: `${plural(shop.late, "order")} running late`, body: "Past their pickup day and still in the kitchen. Call the clients.", cta: "See late orders", to: "/admin/orders?due=late" }
    ...
    <AdminPage
      ...
      status={
        <>
          ...
          {fmtDayLong(now)} · {open.label} · {shop.inProduction} in the kitchen
        </>
      }
      actions={
        <>
          ...
          <div className="adm-stack">
            ...
            <motion.div className="adm-grid adm-grid-2" {...(first ? enter(16, 0.14) : {})}>
              <section className="adm-card" aria-labelledby="visits">
                <CardHead id="visits" title="Today's pickups and sessions" action={<Link to="/admin/appointments" className="adm-link">Calendar <ChevronRight size={14} /></Link>} />
                {visits.length ? (
                  ...
                  <EmptyState icon={<CalendarDays size={22} />} title="Nothing booked today" body="Pickups, deliveries, tastings and design sessions booked for today show here." />
                )}
              </section>
...
/** What the kitchen makes for a day's handovers, with identical cakes counted together. */
    ...
    <section className="adm-card" aria-labelledby="kitchen">
      ...
      <div className="adm-card-foot">
        <span className="adm-meta">
          {count ? `${plural(count, "item")} for ${which === "today" ? "today" : fmtDayShort(next)}. ` : ""}Paid orders only; identical cakes are grouped.
        </span>
      </div>
    </section>
  );
