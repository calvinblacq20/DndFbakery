...
const FILTERS: { id: Purpose; label: string }[] = [
  ...
  { id: "consultation", label: "Design sessions" },
];
      ...
      <div className="adm-with-rail">
        <section className="adm-card" aria-labelledby="day-list">
          ...
          {dayVisits.length ? (
            ...
            <EmptyState icon={<CalendarDays size={22} />} title={closed ? "The bakery is closed this day" : "Nothing booked"} body={closed ? undefined : purpose === "all" ? "Pickups, deliveries, tastings and design sessions booked in the app show here." : "Try another type."} />
          )}
        </section>
                ...
                {requests.map((a) => {
                  ...
                  return (
                    <div key={a.id} className="adm-row" style={{ alignItems: "flex-start" }}>
                      ...
                      <button className="icon-btn" style={{ width: 34, height: 34, background: "var(--ink)", color: "var(--berry)" }} onClick={() => confirm(a)} aria-label="Confirm request">
                        <Check size={17} />
                      </button>
                    </div>
                  );
