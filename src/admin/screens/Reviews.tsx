...
export function Reviews() {
  ...
  const setStatus = (r: Review, status: ReviewStatus) => {
    ...
    notify(status === "published" ? "Review published" : status === "hidden" ? "Review hidden" : "Review moved back", status === "published" ? `${r.name}'s review now shows on the bakery page.` : `${r.name}'s review no longer shows to clients.`);
  };
      ...
      {review && (
        <div className="stack gap-12">
          ...
          <label htmlFor="reply" className="t-cap muted">
            Your reply shows under the review on the bakery page
          </label>
