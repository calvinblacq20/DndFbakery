...
export const STAGES: OrderStatus[] = ["request", "quoted", "confirmed", "baking", "decorating", "ready", "collected"];
...
export type BadgeTone = "berry" | "sand" | "sky" | "lilac" | "mist" | "danger" | "wash";
...
/** Clients can cancel until baking starts. */
...
export function badgeFor(order: Pick<Order, "status" | "payments" | "total" | "readyBy" | "delivery">, now: Date): Badge {
  switch (order.status) {
    ...
    case "ready":
      ...
      return { label: order.delivery === "delivery" ? "Out for delivery" : "Ready for pickup", tone: "berry" };
    case "request":
      return paidTotal(order) > 0 ? { label: "Paid", tone: "berry" } : { label: "Request sent", tone: "lilac" };
    case "quoted":
      return { label: "Action required", tone: "sand" };
    case "confirmed":
      return { label: "Booked", tone: "berry" };
    default:
