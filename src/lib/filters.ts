...
export const STAGE_GROUPS: { id: StageGroup; label: string; statuses: OrderStatus[] }[] = [
  ...
  { id: "production", label: "In the kitchen", statuses: ["confirmed", "baking", "decorating"] },
...
export function parseOrderFilters(params: URLSearchParams): OrderFilters {
  ...
  return {
    ...
    occasions: list(params.get("occasion"), ["birthday", "wedding", "kids", "christening", "bridal", "corporate", "anniversary", "everyday", "other"] as const),
...
/** Name, phone in any Ghana format, order number (DNF-1042, 1042) or menu item. */
export function matchesQuery(order: Pick<Order, "number" | "items">, customer: Pick<Customer, "name" | "phone"> | undefined, query: string): boolean {
  ...
  const numberQuery = q.replace(/^dnf[\s-]*/, "");
