...
const cake = { id: "i1", styleId: "classic-cake", qty: 1, flavour: "red-velvet" as const, size: "9" as const, layers: 3 as const, finish: "buttercream" as const, design: "classic" as const, unitPrice: 500 };
...
const pay = (amount: number, at: string, method: "momo" | "cash" | "bank" | "card" = "momo") => ({ id: `p${amount}${at}`, amount, method, reference: "x", at, receiptNo: "r", kind: "part" as const, receivedBy: "Dough n Frost" });

const clients = [
  { id: "c1", name: "Adwoa Asante", phone: "024 318 7702", email: "", town: "Adenta", memberSince: "", hasAccount: false, points: 0, source: "instagram" as const },
  { id: "c2", name: "Esi Boateng", phone: "055 902 1147", email: "", town: "East Legon", memberSince: "", hasAccount: false, points: 0, source: "walkin" as const },
];

describe("owner stages", () => {
  it("names the exact stage and flags late work, but not work due today", () => {
    expect(ownerBadge(order({ status: "baking" }), NOW)).toEqual({ label: "Baking", tone: "sky" });
    expect(ownerBadge(order({ status: "decorating", readyBy: "2026-09-14" }), NOW)).toEqual({ label: "Late · Decorating", tone: "sand" });
  ...
  it("marks ready orders that still owe money as needing attention", () => {
    ...
    expect(ownerBadge(order({ status: "ready", payments: [pay(500, "2026-09-02T10:00:00Z")] }), NOW)).toEqual({ label: "Ready", tone: "berry" });
  });
  ...
  it("picks one next step per stage", () => {
    ...
    expect(ownerNextStep(order({ status: "confirmed" }))).toEqual({ label: "Start baking", kind: "advance", to: "baking" });
    expect(ownerNextStep(order({ status: "decorating" }))).toEqual({ label: "Mark ready", kind: "advance", to: "ready" });
  ...
  it("writes WhatsApp updates in the bakery's voice", () => {
    ...
    expect(text).toContain("DNF-1041");
    expect(text).toContain("GH₵ 200");
    expect(text).toContain("Have a sweet day!");
    expect(updateMessage(order({ status: "quoted" }), undefined, NOW)).toMatch(/^Hi there, the price for your Celebration cake \(DNF-1041\) is GH₵ 500\. Full payment secures your date/);
  });

  it("shows the handover day for open orders and the lateness for late ones", () => {
    expect(ownerDateLine(order({ status: "baking", neededBy: "2026-09-16" }), NOW)).toEqual({ text: "Pickup tomorrow", late: false });
    expect(ownerDateLine(order({ status: "decorating", readyBy: "2026-09-12" }), NOW)).toEqual({ text: "Late by 3 days · was due Sat, 12 Sept", late: true });
  ...
  it("titles orders with the quantity", () => {
    expect(orderTitle(order({ items: [{ ...cake, styleId: "snack-box", qty: 40, size: undefined, layers: undefined, unitPrice: 55 }] }))).toBe("Corporate snack box × 40");
  });
});
...
describe("bake list", () => {
  it("groups identical cakes due the same day and skips unpaid requests", () => {
    const orders = [
      order({ id: "a", number: "DNF-1001", status: "confirmed" }),
      order({ id: "b", number: "DNF-1002", status: "baking", items: [{ ...cake, qty: 2 }] }),
      order({ id: "c", number: "DNF-1003", status: "baking", items: [{ ...cake, flavour: "chocolate" }] }),
      order({ id: "d", number: "DNF-1004", status: "request" }),
      order({ id: "e", number: "DNF-1005", status: "baking", readyBy: "2026-09-21" }),
    ];
    const list = bakeList(orders, "2026-09-20");
    expect(list.map((l) => [l.label, l.qty])).toEqual([
      ["9 inch · Red velvet · 3 layers", 3],
      ["9 inch · Chocolate · 3 layers", 1],
    ]);
    expect(list[0]?.orders).toEqual(["DNF-1001", "DNF-1002"]);
  });
});
...
describe("reminders", () => {
  ...
  it("writes a friendly nudge that mentions the notice", () => {
    ...
    expect(text).toContain("3 working days' notice");
  });
});
...
describe("metrics", () => {
  const orders = [
    order({ id: "a", createdAt: "2026-09-10T09:00:00", status: "baking", total: 1000, history: [{ status: "request", at: "2026-09-10T09:00:00" }, { status: "quoted", at: "2026-09-10T10:00:00" }, { status: "baking", at: "2026-09-12T10:00:00" }], payments: [pay(500, "2026-09-11T10:00:00", "momo")] }),
  ...
  it("summarises the kitchen and the days ahead", () => {
    const shop = workshopNow([...orders, order({ id: "d", status: "decorating", readyBy: "2026-09-12" }), order({ id: "e", status: "ready" })], NOW);
...
describe("order filters", () => {
  const orders = [
    order({ id: "a", number: "DNF-1042", status: "baking", readyBy: "2026-09-14", customerId: "c1" }),
    order({ id: "b", number: "DNF-1043", status: "ready", readyBy: "2026-09-16", customerId: "c2", payments: [pay(500, "2026-09-02T10:00:00Z")] }),
    order({ id: "c", number: "DNF-0999", status: "collected", readyBy: "2026-08-01", customerId: "c2", payments: [pay(500, "2026-08-02T10:00:00Z")] }),
    order({ id: "d", number: "DNF-1044", status: "quoted", readyBy: "2026-09-30", customerId: "c1", occasion: "wedding", items: [{ ...cake, styleId: "wedding-cake", size: undefined, layers: undefined, tiers: 3, unitPrice: 2800 }], total: 2800 }),
  ];
  ...
  it("ignores junk in the URL", () => {
    const f = parseOrderFilters(new URLSearchParams("stage=bogus,ready&pay=free&owedMin=-4&sort=zzz&occasion=church,kids"));
  ...
  it("finds orders by client name, phone in any format, order number or item", () => {
    ...
    expect(matchesQuery(orders[0]!, adwoa, "dnf-1042")).toBe(true);
