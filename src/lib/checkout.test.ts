...
describe("order access", () => {
  ...
  it("finds an order only when the number and WhatsApp number both match", () => {
    expect(normalizeOrderNumber("dnf1041")).toBe("DNF-1041");
    expect(normalizeOrderNumber("998")).toBe("DNF-0998");
...
describe("payments", () => {
  ...
  it("builds a Paystack reference from the order number", () => {
    expect(paystackReference("DNF-1042", "7k2m9q")).toBe("DNF1042-7K2M9Q");
  });

  it("marks a paid request as paid while the design is confirmed", () => {
    const now = new Date("2026-09-15T10:00:00Z");
    expect(badgeFor({ ...order, payments: [paid(600)] }, now)).toEqual({ label: "Paid", tone: "berry" });
