...
const draft = (overrides: Partial<OrderDraft> = {}): OrderDraft => ({
  ...
  readyBy: "2026-10-10",
  shortNotice: false,
  items: [{ styleId: "classic-cake", qty: 1, flavour: "red-velvet", size: "9", layers: 3, finish: "buttercream", design: "classic", message: "Happy 30th Ama", unitPrice: 500 }],
  total: 500,
  designPlan: "ours",
  handoverStart: "2026-10-10T15:00",
  delivery: "pickup",
  contact: { name: "Ama Mensah", phone: "024 851 5773", email: "ama@gmail.com", town: "Adenta", address: "", digitalAddress: "" },
  remember: true,
  payChoice: "later",
  ...overrides,
});
...
describe("guest checkout", () => {
  ...
  it("books a price-list cake straight away when it's paid in full online", () => {
    ...
    expect(result.payment?.reference).toMatch(/^DNF\d{4}-[A-Z0-9]{6}$/);
  ...
  it("books a design session when the client asks for one", () => {
    ...
    expect(sessions.sort()).toEqual(["consultation", "pickup"]);
  });
