...
const cake = { styleId: "classic-cake", qty: 1, flavour: "vanilla" as const, size: "7" as const, layers: 1 as const, finish: "buttercream" as const, design: "classic" as const, unitPrice: 230 };

describe("sample bakery book", () => {
  ...
  it("numbers orders and receipts once each, in order", () => {
    ...
    expect(numbers.every((n) => /^DNF-\d{4}$/.test(n))).toBe(true);
  ...
  it("prices every sample cake from the price list", () => {
    for (const order of getAppData().orders) {
      for (const item of order.items) {
        if (styleById(item.styleId)?.kind !== "cake" || !item.layers || !item.size) continue;
        expect(gridPrice(item.flavour, item.layers, item.size)).not.toBeNull();
      }
    }
  });
  ...
  it("has a live pipeline for the owner's Today screen", () => {
    ...
    const missing = ["request", "quoted", "confirmed", "baking", "decorating", "ready", "collected", "cancelled"].filter((s) => !statuses.has(s as never));
...
describe("owner actions", () => {
  ...
  it("sends a quote, then full payment books the date and issues the next receipt", () => {
    ...
    expect(part.payment).toMatchObject({ reference: "Cash", kind: "part", receivedBy: "Dough n Frost" });
  ...
  it("books a request that was already paid in full at checkout as soon as it's quoted", () => {
    const placed = actions.placeOrder({
      ...
      contact: { name: "Ama Mensah", phone: "020 111 2233", email: "ama@gmail.com", town: "Adenta", address: "", digitalAddress: "" }, remember: false, payChoice: "now",
  ...
  it("won't hand over an order while it owes, unless the owner allows it", () => {
    ...
    expect(studio.moveTo(quoted.id, "baking", now)).toEqual({ error: "This order is closed." });
  });
  ...
  it("takes a WhatsApp order for a new client, paid in full", () => {
    const draft: WalkInOrder = {
      newClient: { name: "  Ama   Owusu ", phone: "020 111 2233", town: "Adenta", source: "whatsapp" },
      occasion: "birthday",
      neededBy: "2026-10-01",
      readyBy: "2026-10-01",
      shortNotice: false,
    ...
    expect(result.order).toMatchObject({ status: "confirmed", number: `DNF-${orderCounter}` });
  ...
  it("edits an item's price and hides it from clients without touching past orders", () => {
    const before = getAppData().styles.find((s) => s.id === "fudge-loaf")!;
    const sold = getAppData().orders.find((o) => o.items.some((i) => i.styleId === "fudge-loaf"));
    const soldPrice = sold?.items.find((i) => i.styleId === "fudge-loaf")?.unitPrice;

    const saved = studio.saveStyle("fudge-loaf", { fromPrice: 180, readyDays: 3 });
    if ("error" in saved) throw new Error(saved.error);
    expect(styleById("fudge-loaf")).toMatchObject({ fromPrice: 180, readyDays: 3 });
    expect(STYLES.some((s) => s.id === "fudge-loaf")).toBe(true);

    studio.saveStyle("fudge-loaf", { active: false });
    expect(STYLES.some((s) => s.id === "fudge-loaf")).toBe(false);
    // Hidden items still resolve, so old orders keep their name and the menu can bring them back.
    expect(styleById("fudge-loaf")?.name).toBe(before.name);
    expect(sold?.items.find((i) => i.styleId === "fudge-loaf")?.unitPrice).toBe(soldPrice);

    studio.resetStyles();
    expect(getAppData().styles.find((s) => s.id === "fudge-loaf")).toMatchObject({ fromPrice: 150, readyDays: before.readyDays });
    expect(STYLES.some((s) => s.id === "fudge-loaf")).toBe(true);
  });

  it("checks an item before saving it", () => {
    expect(studio.saveStyle("fudge-loaf", { name: " " })).toEqual({ error: "Give the item a name." });
    expect(studio.saveStyle("fudge-loaf", { fromPrice: -20 })).toHaveProperty("error");
    expect(studio.saveStyle("fudge-loaf", { fromPrice: 0 })).toEqual({ error: "Enter a price above zero." });
    expect(studio.saveStyle("fudge-loaf", { readyDays: 0 })).toHaveProperty("error");
    expect(studio.saveStyle("nope", { fromPrice: 10 })).toEqual({ error: "We couldn't find that item." });
  });

  it("edits the cake price list for both sides, and resets it", () => {
    const grid = structuredClone(getAppData().priceGrid);
    grid.classic[1]["6"] = 200;
    delete grid.chocolate[3]["10x12"];
    const saved = studio.savePriceGrid(grid);
    if ("error" in saved) throw new Error(saved.error);
    expect(PRICE_GRID.classic[1]["6"]).toBe(200);
    expect(PRICE_GRID.chocolate[3]["10x12"]).toBeUndefined();
    expect(studio.savePriceGrid({ ...grid, classic: { ...grid.classic, 1: { "6": -5 } } })).toHaveProperty("error");
    studio.resetPriceGrid();
    expect(PRICE_GRID.classic[1]["6"]).toBe(180);
    expect(PRICE_GRID.chocolate[3]["10x12"]).toBe(800);
  });

  it("saves bakery details, hours, fees and policies for both sides", () => {
    const saved = studio.saveSettings({ studio: { phone: "020 999 8877", area: "Adenta Housing Down" }, policies: { payment: "Pay in full to book." }, rules: { lateFee: 80 } });
    if ("error" in saved) throw new Error(saved.error);
    expect(STUDIO.phone).toBe("020 999 8877");
    expect(POLICIES.payment).toBe("Pay in full to book.");
    expect(RULES).toEqual({ lateFee: 80, designFee: 150 });
    expect(getAppData().settings.studio.area).toBe("Adenta Housing Down");
    ...
    expect(STUDIO.phone).toBe("054 155 2128");
    expect(RULES.lateFee).toBe(50);
    expect(HOURS[0]).toBeNull();
  });

  it("checks bakery details before saving them", () => {
    ...
    expect(studio.saveSettings({ studio: { ...current, name: " " } })).toEqual({ error: "The bakery needs a name." });
