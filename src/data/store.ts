...
export interface OrderDraft {
  ...
  /** Design and tasting session, when the plan is "consult". */
  consultStart?: string;
  delivery: Delivery;
  comments?: string;
...
export const actions = {
  ...
  placeOrder(draft: OrderDraft, now = new Date()): { order: Order; payment?: Payment } | { error: string } {
    ...
    if (draft.designPlan === "consult" && draft.consultStart) {
      newAppointments.push({ id: newId("a"), customerId: customer.id, orderId, purpose: "consultation", start: draft.consultStart, minutes: 45, status: "requested" });
    }
...
const SIZE_KEYS: CakeSize[] = ["6", "7", "9", "11", "9x11", "10x12"];
...
export const studio = {
  ...
  /** Saves the cake price list. A blank cell means that combination isn't offered. */
  savePriceGrid(grid: PriceGrid): Result<{ grid: PriceGrid }> {
    const next = cloneGrid(grid);
    let offered = 0;
    for (const group of ["classic", "chocolate"] as const) {
      for (const layers of [1, 2, 3] as const) {
        for (const size of SIZE_KEYS) {
          const price = next[group][layers][size];
          ...
          next[group][layers][size] = Math.round(price);
          offered++;
        }
      }
    }
  ...
  saveSettings(patch: { studio?: Partial<StudioSettings["studio"]>; hours?: StudioSettings["hours"]; policies?: Partial<StudioSettings["policies"]>; rules?: Partial<StudioSettings["rules"]> }): Result<{ settings: StudioSettings }> {
    ...
    if (next.studio.name.length < 2) return { error: "The bakery needs a name." };
    ...
    if (next.studio.email.trim() && !/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(next.studio.email.trim())) return { error: "Enter an email address, like doughnfrost@gmail.com." };
