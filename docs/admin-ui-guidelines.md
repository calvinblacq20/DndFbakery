...
> **Dough n Frost adaptation.** This reference came from the Dough n Frost build, and this app reuses its layout, components and motion unchanged. What differs: the palette (cocoa ink `#2B1F1D` on warm cream `#F5EDE8`, frosting pink `#FF9CCB` as the one accent, called `berry` in code where the tailor build said `lime`, and bakery pastels in the same slots; see `src/styles/tokens.css`), the frosted-doughnut mark (`src/components/Brand.tsx`), and the vocabulary:
>
> | Tailor build | Dough n Frost |
> |---|---|
> | Styles, catalogue | Menu items, the menu (plus the cake price list) |
> | Measurements, measuring visit | Dates to remember, allergies; design and tasting session |
> | Fitting | Pickup or delivery slot |
> | Deposit (50%) | Full payment secures the order |
> | Cutting · Sewing · Fitting | Baking · Decorating |
> | Rush (+20%) | Late order (+GH₵ 50) |
> | Workshop | Kitchen |
> | Instagram| Instagram |

Rule for the owner side: **the look comes from the client side, the structure from the dashboard references in `STRUCTURE REFERENCE/admin/`, and the content from the owner's daily work at Dough n Frost.**
...
| Brand | The Dough n Frost mark in the sidebar and tab bar. Splash stays strictly black and white. |
...
```
┌───────────────────┬──────────────────────────────────────────────────────────────┐
│ [DnF] Dough n Frost │  Today              [ Search clients, orders… ]  (!) (F)     │
...
- Logo row at the top: `LogoMark` 32px + "Dough n Frost" (`t-title`) + "Bakery admin" (`t-cap`, ink-75).
...
- Metrics for Dough n Frost: **Cash received · New orders · Balance owed · Collected** (Today); **New orders · Collected · Average order · On time %** (Reports). "Due this week" has no history to chart, so it lives in the workshop card instead.
